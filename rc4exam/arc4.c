...
static byte MakeByte(word32* x, word32* y, byte* s)
{
    word32 a = s[*x], b;
    *y = (*y+a) & 0xff;
...
{
    int length = 8;
    //const unsigned char indata[8] = {0x01, 0x23, 0x45, 0x67, 0x89, 0xab, 0xcd, 0xef};
    //const unsigned char key[8] = {0x01, 0x23, 0x45, 0x67, 0x89, 0xab, 0xcd, 0xef};
    char *indata = "input data for test";
    int data_length = strlen(indata);
    char *key = "testkeyl";
    unsigned char out[length], out2[length];
    struct Arc4 arc4;
    Arc4SetKey(&arc4, key, length);
    Arc4Process(&arc4, out, indata, data_length);
    int i;
    ...
    Arc4SetKey(&arc4, key, length);
    Arc4Process(&arc4, out2, out, data_length);
    for(i=0; i<data_length; i++)
    {
        printf("%c", out2[i]);
    }
